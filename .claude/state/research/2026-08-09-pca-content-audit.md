...
- [x] `questions/section-1-designing-planning.md:182` Q10 - "no Spot VM option for Dataflow workers". Dataflow FlexRS uses preemptible VMs for most of the batch pool. Rewritten: FlexRS defaults to preemptible VMs for 90% of the worker pool, and the reason to reject option B is now the documented six-hour scheduling window, which cannot meet the stem's 4-hour deadline. Verified against https://cloud.google.com/dataflow/docs/guides/flexrs
...
- [x] `questions/section-1-designing-planning.md:719` Q41 - "Vertex AI Endpoints do not support scaling to zero with GPUs". The false claim has been removed; option B is now rejected on what it actually configures (it pins a minimum of one replica, so four A100s are billed around the clock). The positive claim that `min_replica_count=0` supports GPU deployments could not be confirmed on `cloud.google.com` in this pass, so it is not asserted in the file. See the open-verification list below.
- [x] `questions/section-2-provisioning-infrastructure.md:177` Q10 - "Standard Tier does not support Cloud Armor". Confirmed against the Cloud Armor overview, which states support for load balancers in either Premium Tier or Standard Tier. Option E is now rejected on the real grounds (a tier change does not mitigate an L7 attack and would give up the global anycast frontend), with an explicit note that the tier claim is false.
- [x] `questions/section-2-provisioning-infrastructure.md:192` Q11 - justifies Nearline for quarterly access. Google maps monthly to Nearline, quarterly to Coldline. Rewritten to quote Google's own access-frequency wording and minimum storage durations, to say plainly that the stem's middle tier maps to Coldline in isolation, and to explain that option C still wins because the over-365-day tier holds most of the volume and Archive is correct there.
...
- [x] `questions/section-4-optimizing-processes.md:433` Q16 - built on `terraform refresh`, deprecated in favour of `apply -refresh-only`. Option B, the explanation and the exam tip now use `terraform apply -refresh-only`, and the deprecation is called out explicitly. Verified against HashiCorp's own command page, which does not name a version, so the version number is not asserted in the file.
- [x] `questions/section-6-operations-excellence.md:87` Q3 - streaming inserts priced at "$0.01/200KB". Corrected to $0.01 per 200 mebibyte with a 1 KB minimum per row, and the free batch-loading comparison now names the shared `default-pipeline` slot pool. Both figures read off the live BigQuery pricing table.
- [x] `questions/section-6-operations-excellence.md:354` Q13 - "escalation policy in Cloud Monitoring". Option B and the explanation now put the escalation policy in the incident-management notification channel, and state that Cloud Monitoring has no native escalation tier. Notification channel list verified against the Monitoring notification options page.
- [x] `questions/section-6-operations-excellence.md:493` - typo "addatively" in the composite SLA explanation. Sentence rewritten. The option/explanation mismatch in the same question was fixed at the same time: option B now shows `0.9995 x 0.9995 x 0.9999`, matching the arithmetic the explanation performs.
- [ ] `questions/case-study-questions.md:384` Q14 - "Autopilot doesn't support Spot VMs". Autopilot supports Spot Pods.
- [x] `questions/section-4-optimizing-processes.md:174` Q7 and `section-6-operations-excellence.md:258` Q10 - conflate Skaffold verify with Cloud Deploy deploy analysis. Both marked-correct options and both explanations now name deploy analysis (telemetry and alerting policies over a duration) plus a `repairRolloutRule` automation for the rollback, and both exam tips draw the line against deployment verification, which runs a container you supply. https://cloud.google.com/deploy/docs/analysis
...
- [?] **Vertex AI to Gemini Enterprise Agent Platform.** Rebranded at Cloud Next 2026; Vertex AI left the Console 2026-05-21. "Vertex AI Agent Builder" appears as a *correct answer* in `section-1-designing-planning.md:659` and `:1094`, `section-2-provisioning-infrastructure.md:661`, `case-study-questions.md:207`. Model tables reference SKUs that now 404 (`docs/02:2098-2100`, `02:2190`, `01:787`). Affects docs 01, 02, 09.
  ...
  **Applied in the question files (2026-08-09):** the recommendation is now implemented in `questions/section-1` and `questions/section-2`. Vertex AI remains the primary name everywhere, with a short "now marketed as Gemini Enterprise Agent Platform" note where the name carries a marked-correct answer (section-1 Q11, Q38, Q59; section-2 Q34). No blanket rename was done. The docs files are untouched by this lane.
- [ ] **Anthos to GKE Enterprise**, **Anthos Service Mesh + Traffic Director to Cloud Service Mesh**: **question files done** (`questions/section-1` Q16, Q37, Q60 and `questions/section-2` Q25: marked-correct option text and explanations now use Cloud Service Mesh, GKE Enterprise and Config Sync, with the old names kept in parentheses and in the exam tips so the mapping is learnable). **Docs still open**: `docs/01:279`, `01:641`, `01:742`, `01:1043`, `02:425`, `02:1505`, `04:429`, `04:668`, `04:749`, `04:790`, `04:820`, `04:972`, `06:788`, `06:1586`, `06:1660`, `06:1674`, `07:102`, `07:469`, `07:1005`, `07:1388`, `05:115`.
...
- [ ] **Cloud Functions to Cloud Run functions**: **question files handled** -- `questions/section-5-managing-implementations.md:229` and `:531` are both about the Firebase Local Emulator Suite, where the product genuinely remains "Cloud Functions for Firebase", so a blanket rename would have introduced a new error. Both exam tips now carry an explicit naming note (Google Cloud product is Cloud Run functions, Firebase side keeps Cloud Functions for Firebase), and `questions/section-2` Q21 and `questions/section-1` Q58 name Cloud Run functions. **Docs still open**: `docs/01:1018`, `05:169`, `05:216`, `05:238`, `06:622`, `06:641`, `07:562`, `07:977`, `07:803`, `07:1109`, `07:1231`.
...
- [x] **BeyondCorp Enterprise to Chrome Enterprise Premium**: `questions/section-3-security-compliance.md:749`. Rename verified by redirect: `cloud.google.com/beyondcorp-enterprise/docs` now resolves to `chrome-enterprise-premium/docs`. The Q39 explanation names Chrome Enterprise Premium with the old name in parentheses.
...
- [ ] **Model Armor is under Security Command Center**, not Vertex AI: `questions/section-3-security-compliance.md:654` **done**, `docs/03:650-654` (also wrong doc URL) **still open**. The question file now states that Model Armor is a standalone service documented under Security Command Center rather than a Vertex AI feature, lists what it screens (responsible-AI categories, prompt injection and jailbreak, sensitive data, malicious URLs), and links https://cloud.google.com/security-command-center/docs/model-armor-overview. The `docs/03` half belongs to the docs lane.
...
- [x] **Sections 1 and 2 carry 110 questions, 42% of the bank, with no exam tips and no doc links at all.** Fixed 2026-08-09. All 110 answer blocks were rewritten into the sections 4/5/6 shape: lead paragraph, per-option `- **X) is wrong** -- reason` bullets, `**Exam tip:**`, then a `Docs:` line. Counts now read 65/65/65 and 45/45/45 for questions, tips and doc links.
- [x] Sections 3 and case-study have tips but no doc links. Section 3 now carries a `Docs:` line on all 45 questions; case-study already had 20 after its rebuild. Every URL used across the six section files was HTTP-checked, and 14 pre-existing dead links in sections 4, 5 and 6 were repointed at verified pages in the same pass.
- [ ] Structural split: sections 1, 2, 3 and case-study put the stem inline on the `### Qn.` line; sections 4, 5 and 6 put it on the following line. **Partly closed:** the answer-block shape is now uniform across all seven files (bullets, tip, `Docs:`). What remains is only the stem placement -- sections 1, 2, 3 and case-study still carry the stem on the `### Qn.` line, so a parser keying off `### Q` still needs two cases.
...
- [ ] Vertex AI scale to zero for GPU deployments. `min_replica_count = 0` on a GPU-backed endpoint could not be confirmed on `cloud.google.com` in the 2026-08-09 pass: the inference compute-resources page returned only navigation content and `predictions/use-autoscaling` and `predictions/scale-to-zero` both 404. The false negative claim was removed from `questions/section-1:719`, but the positive claim is not asserted anywhere and should not be added to any file until a documentation page states it.
