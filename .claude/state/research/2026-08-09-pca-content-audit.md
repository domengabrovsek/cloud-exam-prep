...
- [ ] **The 20 case study questions need rebuilding on the real cases.** Two problems, now both confirmed. First, every needed constraint is restated in the stem, so all 20 are answerable without opening the case study. Second, and worse, the invented numbers come from the fabricated `docs/08`, so several test facts that do not exist: `:69`, `:121`, `:151`, `:289`, `:423`. Q1 hinges on .NET Framework 4.x (not in EHR), Q7 on Oracle PL/SQL at 50,000 TPS (Cymbal has no Oracle), Q11 on a 2.5 PB Transfer Appliance move (Altostrat's library is already in Cloud Storage), Q19 on IBM z/OS and COBOL transaction volumes (KnightMotives names neither). Q5 and Q11 also duplicate `section-4:288` and `section-2:284`; Q9 restates `section-5:11`.
  Rebuild by quoting a real requirement in each stem and keeping two options plausible until that requirement is applied. Drop the arithmetic: the official cases contain one number in total.
  A warning pointing at this has been added to the bottom of `docs/08` so the questions are not trusted in the meantime.
